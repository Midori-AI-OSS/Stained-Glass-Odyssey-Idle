...
@dataclass
class LadyStormSupercellBoss(LadyStormSupercell):
    """Boss Supercell tuned to 1.5× baseline wind/lightning bonuses."""

    plugin_type = "passive"
    id = "lady_storm_supercell_boss"
    name = "Supercell (Boss)"
    ...
    async def apply(
        ...
        **_: object,
        ...
        storm_edge_effect = StatEffect(
            ...
            stat_modifiers={
                "atk": int(base_attack * 0.33),
                "crit_rate": 0.18,
                "effect_hit_rate": 0.225,
            },
            duration=-1,
            source=self.id,
        )
        ...
        storm_barrier_effect = StatEffect(
            ...
            stat_modifiers={
                "mitigation": 0.18,
                "effect_resistance": 0.18,
                "defense": int(base_defense * 0.225),
            },
            duration=-1,
            source=self.id,
        )
        ...
        if foes:
            for foe in foes:
                ...
                squall_marker = StatEffect(
                    ...
                    stat_modifiers={"dodge_odds": -0.03},
                    duration=2,
                    source=self.id,
                )
    ...
    async def on_action_taken(self, target: "Stats", **kwargs: object) -> None:
        ...
        if phase == "wind":
            gale_effect = StatEffect(
                ...
                stat_modifiers={"spd": 15, "dodge_odds": 0.09},
                duration=2,
                source=self.id,
            )
            ...
            for ally in allies:
                ...
                ally_effect = StatEffect(
                    ...
                    stat_modifiers={"spd": 9, "dodge_odds": 0.06},
                    duration=2,
                    source=self.id,
                )
        ...
        surge_effect = StatEffect(
            ...
            stat_modifiers={
                "crit_rate": 0.075 * new_charge,
                "crit_damage": 0.12 * new_charge,
            },
            duration=1,
            source=self.id,
        )
        ...
        for foe in foe_list:
            ...
            shock_effect = StatEffect(
                ...
                stat_modifiers={"mitigation": -0.06 * new_charge},
                duration=1,
                source=self.id,
            )
    ...
    async def on_hit_landed(
        ...
        **_: object,
        ...
        weapon_floor = (charges * 225) // 2
        atk_scale = int(attacker.atk * (0.45 + 0.225 * (charges - 1)))
        bonus_damage = max(atk_scale, weapon_floor)
        ...
        try:
            await target_hit.apply_damage(
                ...
                action_name="Boss Supercell Surge",
            )
        except Exception:
            pass
        ...
        shock_lock = StatEffect(
            ...
            stat_modifiers={
                "mitigation": -0.09 * charges,
                "dodge_odds": -0.03 * charges,
            },
            duration=2,
            source=self.id,
        )
        ...
        regen_effect = StatEffect(
            ...
            stat_modifiers={"regain": charges * 45},
            duration=3,
            source=self.id,
        )
    ...
    async def _spread_gale(
        ...
        members = party.members if hasattr(party, "members") else party
        for ally in members:
            if ally is target or getattr(ally, "hp", 0) <= 0:
                continue
            gale = StatEffect(
                name=f"{self.id}_everstorm_aura_{id(target)}",
                stat_modifiers={"spd": 6, "regain": 30},
                duration=3,
                source=self.id,
            )
    ...
    def get_description(cls) -> str:
        return (
            "[BOSS] Tailwinds grant +12 SPD, +12% dodge, and heavier aggro reduction while alternation buff/debuff values sit halfway between normal and glitched tiers. Charges add +7.5% crit/+12% crit damage per stack and detonate for hefty strikes that shred mitigation by 9% per charge."
        )
