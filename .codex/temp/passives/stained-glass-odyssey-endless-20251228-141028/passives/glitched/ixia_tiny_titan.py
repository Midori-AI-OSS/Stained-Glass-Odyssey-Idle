...
@dataclass
class IxiaTinyTitanGlitched:
    """Glitched variant of Ixia's Tiny Titan with doubled Vitality gains.

    Glitched Ixia gains Vitality twice as fast (0.02 per hit instead of 0.01),
    resulting in more rapid scaling but also faster defense loss. The risk-reward
    balance is more volatile.
    """
    plugin_type = "passive"
    ...
    async def apply(self, target: "Stats") -> None:
        """Apply Ixia's Tiny Titan mechanics with doubled gains."""
        entity_id = id(target)
        ...
        # Glitched: Double Vitality gain (0.02 instead of 0.01)
        self._vitality_bonuses[entity_id] += 0.02

        # Compute effective stacks from accumulated vitality bonus
        ...
        # Convert 500% of current Vitality into attack based on BASE atk
        try:
    ...
    def get_description(cls) -> str:
        return (
            "[GLITCHED] Taking damage increases Vitality by 0.02 permanently (doubled). "
